...
Invest::Invest(wxWindow* parent, wxPanel* main_panel): wxPanel(parent, wxID_ANY)
{
	...
	wxButton* buttontap1 = new wxButton(menutap, wxID_ANY, L"🏚️");
	...
	buttontap2->Bind(wxEVT_BUTTON, [this](wxCommandEvent&) {
	...
	mainScroll->SetScrollRate(0, 20);
		...
		buttontap1->Bind(wxEVT_BUTTON, [this](wxCommandEvent&) {
			this->Hide();
			mainpanel->Show();
			mainpanel->GetParent()->Layout(); // เช่อมหน้าหลัก
			});
