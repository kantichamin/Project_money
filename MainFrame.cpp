	...
	: wxFrame(NULL, wxID_ANY, title, wxDefaultPosition, wxSize(800, 750))
{
	...
	//หัวข้อแอป
	wxStaticText* header = new wxStaticText(mainPanel, wxID_ANY, L"Rainy Day");
	...
	m_listCtrl = new wxListCtrl(mainPanel, wxID_ANY, wxDefaultPosition, wxSize(400, 450), wxLC_REPORT);
	m_listCtrl->SetBackgroundColour(wxColour(45, 45, 45));
	m_listCtrl->SetForegroundColour(*wxWHITE);

	m_listCtrl->InsertColumn(0, L"วันที่", wxLIST_FORMAT_LEFT, 90);
	m_listCtrl->InsertColumn(1, L"รายละเอียด", wxLIST_FORMAT_LEFT, 140);
	m_listCtrl->InsertColumn(2, L"ประเภท", wxLIST_FORMAT_LEFT, 80);
	m_listCtrl->InsertColumn(3, L"จำนวนเงิน", wxLIST_FORMAT_RIGHT, 90);
